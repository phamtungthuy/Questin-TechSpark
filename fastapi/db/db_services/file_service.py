...
from db.constants import FileType, KNOWLEDGEBASE_FOLDER_NAME, FileSource, ParserType

from services.conn.storage_factory import STORAGE_IMPL

from utils import get_uuid
from utils.file_utils import filename_type, thumbnail_img
from db.db_utils import duplicate_name
...
class FileService(CommonService):
    ...
    def get_by_pf_id(cls, tenant_id, pf_id, page_number, items_per_page,
        ...
        """
        Retrieves files by parent folder ID with pagination and optional keyword filtering.
        Args:
            tenant_id (int): The ID of the tenant.
            pf_id (int): The parent folder ID.
            page_number (int): The page number for pagination.
            items_per_page (int): The number of items per page for pagination.
            orderby (str): The field by which to order the results.
            desc (bool): Whether to order the results in descending order.
            keywords (str): Optional keywords to filter the file names.
        Returns:
            tuple: A tuple containing:
                - list: A list of dictionaries representing the files.
                - int: The total count of files matching the criteria.
        """
        if keywords:
    ...
    def get_by_pf_id_name(cls, id, name):
        """
        Retrieve a file by its parent ID and name.

        Args:
            id (int): The parent ID of the file.
        ...
        Raises:
            RuntimeError: If there is a database error during file retrieval.
        """
    ...
    @classmethod
    @DB.connection_context()
    def create_folder(cls, user_id, file, parent_id, name, count):
        if count > len(name) - 2:
            return file
        else:
            file = cls.insert({
    ...
    @classmethod
    @DB.connection_context()
    def upload_document(self, cluster, kb, file_objs, user_id):
