...
from services.conn import singleton

class Payload:
    """
    Payload class for handling messages from a Redis stream.
    Attributes:
        __consumer: The Redis consumer instance.
        __queue_name: The name of the Redis stream queue.
        __group_name: The name of the consumer group.
        __msg_id: The ID of the message in the stream.
        __message: The parsed JSON message.
    Methods:
        ack():
            Acknowledges the message in the Redis stream.
            Returns True if the acknowledgment is successful, otherwise logs a warning and returns False.
        get_message():
...
class RedisDB:
    ...
    def queue_consumer_batch(self, queue_name, group_name, consumer_name, msg_id=">", batch_size=10) -> list[Payload]:
        try:
            try:
                group_info = self.REDIS.xinfo_groups(queue_name)
                if not any(e["name"] == group_name for e in group_info):
            ...
            if 'key' in str(e):
                return []
            logging.warning(f"[EXCEPTION]consumer_batch: {queue_name}||{str(e)}")
            self.__open__()
        return []
REDIS_CONN = RedisDB()