...
class AgentWorkFlow:
    ...
    def hello(self, query: str):
        system_prompt = """
Bạn là tư vấn viên ảo, được phát triển bởi nhóm nghiên cứu iSE của Khoa CNTT - Trường ĐH Công nghệ. Hãy thực hiện các bước sau:
...
- Nếu câu hỏi có chỉ định năm: giữ nguyên
...
- Thời gian hiện tại là {current_date}
- Nếu câu hỏi có chỉ định năm: giữ nguyên
- Nếu không: Hãy xem xét 2 năm {previous_year} và {current_year}.
...
- Giới hạn 500 từ
- Cố gắng viết các thông tin liên qua tới nhau trên 1 dòng.
...
"""
    ...
    async def _handle_retrieval_flow(self, query, tools, merged_query, history, think_msg, validator=False):
        ...
        if tools[0]["name"] == "next":
            ...
            async for response in self._generate_answer(query, "\n\n\n".join(new_knowledges), "", sum_history, new_ranks, think_msg=think_msg):
                yield response
