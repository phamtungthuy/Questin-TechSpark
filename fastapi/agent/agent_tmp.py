...
"""
    ...
    async def retrieval(self, questions: list[object], top_k=None):
        ...
        # Sử dụng threading.Lock vì hàm sẽ chạy trong executor (thread riêng)
        lock = threading.Lock()  
        ranks = {"total": 0, "chunks": [], "doc_aggs": {}}
        top_n = top_k if top_k else self.dialog.top_n

        def _retrie_sync(query, external_source):
            # Gọi hàm đồng bộ retrieval
            kbinfos = retrievaler.retrieval(
                query, self.embd_mdl, self.dialog.tenant_id, self.dialog.kb_ids, 
                1, top_n, self.dialog.similarity_threshold, 
                self.dialog.vector_similarity_weight, None, self.dialog.top_k, 
                False, None, external_source
            )
            # Đồng bộ hóa với lock (vì đây là hàm đồng bộ đang chạy trong executor)
            print(kbinfos["total"])
        ...
        # Tạo tasks chạy hàm đồng bộ trong executor
        tasks = [
            loop.run_in_executor(None, _retrie_sync, q["query"], q["external_source"])
            for q in questions
        ]
    ...
    async def _handle_retrieval_flow(self, query, tools, merged_query, history, think_msg):
    ...
    async def _run(self, query: str, history: list[object]):
        ...
        async for response in self._handle_retrieval_flow(query, tools, merged_query, history, think_msg):
            yield response
...
async def main():
    ...
    dialog_id = "f6b92e08d16511ef831c0242ac120006"
