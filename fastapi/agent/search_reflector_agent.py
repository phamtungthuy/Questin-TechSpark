...
class SearchReflectorAgent(Agent):
    def __init__(self, model, tools, system=None):
        if not system:
            system = """You are a smart reflector assistant. Your task is to review the retrieved information and evaluate its sufficiency in answering the query accurately.  
                        If knowledges is sufficient, set `enough` = `true` and leave `supporting_questions` empty.
